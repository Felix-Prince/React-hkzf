...
class CityList extends Component {
    ...
    calRowHeight = ({ index }) => {
        ...
        return index === cityIndex.length - 1 ? window.innerHeight : TITLE_HEIGHT + list.length * CITY_HEIGHT
    }
    ...
    renderIndex() {
        return (
            this.state.cityIndex.map((item, index) => (
                <li className="city-index-item" key={item} onClick={() => this.scrollIndex(index)}>
                    {/* index-active */}
                    <span className={index === this.state.activeIndex ? 'index-active' : ''}>{this.formatIndex(item)}</span>
                </li>
            ))
        )
    }
